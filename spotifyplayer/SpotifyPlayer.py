...
class SpotifyApp:
	def __init__(self):
		closeOld = lambda: Application().connect(title=WIN_TITLE).kill(soft=True)
		try: closeOld()
...
class ScreenInfo:
	def __init__(self):
		...
		screen = {i: monInfo.get('Monitor')[n] for n,i in enumerate(['x','y','w','h'])}
		self.taskbarH = screen['h']-monInfo.get('Work')[3]
		self.X = round(screen['w'] - LBL_W*2 - BTN_SIZE*3 - OFFSET_X)
		self.Y = round(screen['h'] - max(BTN_SIZE, (self.taskbarH+BTN_SIZE)/2) - OFFSET_Y)
...
class PlayerMenu:
	def __init__(self, root):
		...
		self.movemenu.add_command(label="Move (Horizontal)", command=lambda: self.movePlayer("X"))
		self.movemenu.add_command(label="Move (Vertical)", command=lambda: self.movePlayer("Y"))
		self.movemenu.add_separator()
		self.movemenu.add_command(label="Save Position", command=self.savePos, state=DISABLED)
		self.movemenu.add_command(label="Reset Position", command=self.resetPos, state=DISABLED)
		self.mainmenu.add_cascade(label="Move Player", menu=self.movemenu)
		self.mainmenu.add_separator()
		self.mainmenu.add_command(label="Exit Spotify", command=root.spotify.win.close)
		self.mainmenu.add_command(label="Close Player", command=root.closePlayer)
		self.mainmenu.add_separator()
		self.mainmenu.add_command(label="Minimize Player", command=root.trayicon.minimizePlayer)

	def movePlayer(self, axis):
		self.axis = axis
		[c.unbind('<ButtonRelease-1>') for c in self.root.mediaBtns]
		self.root.config(cursor="fleur")
		self.startX, self.startY = [round(self.root.winfo_rootx()), round(self.root.winfo_rooty())]
	...
	def on_move(self, event):
		cX,cY = self.xy
		self.dX = OFFSET_X_STEP * round((event.x-cX)/OFFSET_X_STEP) if self.axis == "X" else 0
		self.dY = 0 if self.axis == "X" else OFFSET_Y_STEP * round((event.y-cY)/OFFSET_Y_STEP)
		self.tw.geometry('+{}+{}'.format(self.startX+self.dX, self.startY+self.dY))
	...
	def stopMoving(self, event=None):
		...
		[c.bind('<ButtonRelease-1>', lambda e,n=n: self.root.sendInput(e,n)) for n,c in enumerate(self.root.mediaBtns)]
	...
	def savePos(self):
		...
		for line in fileinput(__file__, inplace=True):
			line = line.rstrip('\r\n')
			out = '{} = {}'.format(s, r) if (r:=change.get(s:=line.split(' = ')[0])) != None else line
			print(out)
	...
	def resetPos(self):
		...
		self.movemenu.entryconfig(5, state=DISABLED)
		self.root.focus_set()
		self.root.geometry('+{}+{}'.format(self.root.screen.X, self.root.screen.Y))
...
class TrayMenu(TrayIcon):
	def __init__(self, root):
		...
		TrayIcon.__init__(self, 'Spotify Player Icon', TRAY_ICON_IMG,
						  ...
						  'Separator',
						  ("Minimize Player", self.minimizePlayer, {'checked': lambda i: self.is_mini}),
...
class GUI(Tk):
	def __init__(self):
		...
		self.geometry('{}x{}+{}+{}'.format(BTN_SIZE*3 + LBL_W*2, BTN_SIZE, self.screen.X, self.screen.Y))
		...
		self.bind_all('<ButtonRelease-3>', lambda e: pMenu.mainmenu.post(e.x_root, self.winfo_rooty()))
	...
	def createPlayer(self):
		# Create Buttons
		pvBtn = [23, 124, 142, 56, 142, 89, 199, 54, 199, 194, 142, 162, 142, 196]
		stBtn = [70, 70, 180, 70, 180, 180, 70, 180]
		plBtn = [83, 54, 204, 124, 83, 196]
		nxBtn = [225, 124, 106, 192, 106, 159, 49, 194, 49, 54, 106, 86, 106, 52]
		self.mediaBtns = [self.createButton(i, [round(BTN_SIZE*n/248) for n in lst]) for i,lst in enumerate([pvBtn, stBtn, plBtn, nxBtn])]
	...
	def createButton(self, i, coords):
		...
		cnv.bind('<Enter>', lambda e, c=cnv, o=cir, p=poly: [c.itemconfig(o, outline='lightgray'), c.itemconfig(p, outline='lightgray', fill='lightgray')])
		cnv.bind('<Leave>', lambda e, c=cnv, o=cir, p=poly: [c.itemconfig(o, outline='white'), c.itemconfig(p, outline='white', fill='white')])
	...
	def updateText(self):
		def initText():
			try:
				winTimings.wait_until(5, 0.5, self.spotify.win.window_text, WIN_TITLE)
				sleep(2)
				self.spotify.win.send_keystrokes('^{UP}') # make sure the volume isn't 0
		...
		while True:
			...
			if not self.showPlayer: break
			if (curTitle := self.spotify.win.window_text()) == title: continue
			# else update player
