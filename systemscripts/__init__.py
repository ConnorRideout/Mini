"""Scripts for use from the command line"""
...
class _run:
    def __init__(self):
        self.buildParser()
        self.subs = {}
        allFuncs = {na: fn for na, fn in globals().items() if na[0] != '_'}
        for funcName in [f[:-3] for f in os_listdir(os_path.dirname(__file__)) if f[0] != '_' and f[-3:] == '.py']:
            self.createHelp(funcName, allFuncs[funcName])
        self.getArgs()
